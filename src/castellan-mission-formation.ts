...
export interface CastellanPredicateAssessment { dossierRef: string; handoffRef: string; determinations: PredicateDetermination[]; }
...
export class CastellanOperatingLayer {
  recordAssessment(dossier: GovernedArtifactEnvelope<MissionDossier>, handoff: GovernedArtifactEnvelope<SecretariatDossierHandoff>, determinations: PredicateDetermination[], context: ArtifactContext = {}): GovernedArtifactEnvelope<CastellanPredicateAssessment> {
    assertDossier(dossier); assertHandoff(dossier, handoff); validateDeterminations(dossier, determinations);
    ...
    return createGovernedArtifact("CastellanPredicateAssessment", "CastellanOperatingLayer", dossier.correlationId, { dossierRef: ref(dossier), handoffRef: ref(handoff), determinations: determinations.map((item) => ({ ...item, values: clean(item.values), rationale: item.rationale.trim() })) }, governance, [ref(dossier), ref(handoff), dossier.payload.inquiryRef!, doctrineRef, lexiconRef, profileRef], context);
  }
}
