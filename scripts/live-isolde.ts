...
async function main(): Promise<void> {
  ...
  try {
    ...
    const access = await openLocksmithOpenAIAccess();
    ...
    stdout.write(`Audit: provider=${result.model}; credential_exposed_to_isolde=false; response_evaluated=false; mission_executed=false; turns=1\n`);
  } finally {
    prompt.close();
  }
}
