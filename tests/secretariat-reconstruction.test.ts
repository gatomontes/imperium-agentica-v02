...
describe("Secretariat reconstruction", () => {
  it("admits a complete exact-v2 Office Profile through Tribunalis and the assigned Senator", () => {
    ...
    expect(SECRETARIAT_PROFILE_CANDIDATE.payload.coreDoctrineRef).toBe(ENACTED_CORE_DOCTRINE_V6.doctrine.identity + "@6");
  ...
  it("opens an exact dossier while preserving raw intent and keeping assertions unproven", () => {
    ...
    expect(dossier.payload.lexiconRef).toBe("imperiumlexicon-core-v1@4");
    expect(dossier.governance).toMatchObject({
      coreDoctrineRef: "coredoctrine-core-v1@6",
      lexiconRef: "imperiumlexicon-core-v1@4",
      officeProfileRef: ADMITTED_SECRETARIAT_PROFILE.identity + "@2",
      vocabularyUses: expect.arrayContaining([
        { termId: "LEX-009", lexiconRef: "imperiumlexicon-core-v1@4", value: "mission_dossier" },
        { termId: "LEX-011", lexiconRef: "imperiumlexicon-core-v1@4", value: "secretariat" },
      ]),
    });
  });
